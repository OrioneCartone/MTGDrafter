...
class ScoringBot(BaseBot):
    ...
    def __init__(self, player: Player):
        ...
        # Pesi per ogni categoria di valutazione.
        self.weights = {
        ...
        # Mappatura degli indici, creata una sola volta per efficienza
    ...
    def pick(self, pack: DraftPack, pack_number: int, pick_number: int) -> Card:
        ...
        for card in pack.cards:
            ...
            # --- Calcolo Punteggi Parziali (logica interna alla classe) ---
            ...
            card_advantage = (self._get_feature(features, 'ability', 'draw_multiple_cards') * 6.0 +
                              self._get_feature(features, 'ability', 'cantrip_etb') * 4.0 +
                              self._get_feature(features, 'ability', 'monarch') * 7.0)

            removal = (self._get_feature(features, 'ability', 'exile_permanent') * 6.0 +
                       self._get_feature(features, 'ability', 'destroy_creature') * 5.0)
            
            # ... puoi aggiungere qui altre categorie che abbiamo definito ...
